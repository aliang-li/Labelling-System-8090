...
<%!String websit = "http://222.24.62.190:8090/BB/jsps/test.jsp?";%>
<%!int fontSize = 0;%>
<%!int number = 0;%>
<%!String name;%>
...
<div style="text-align:center">
		...
		if (!oldName.contains("_")) {
			if (oldFile.isFile()) {
				...
				if (oldFile.renameTo(newFile)) {
					try{
					copyFile(new File("/data/leichao/rename/"),newFile);
		...
		return res;
			...
			/*final String deleteChineseString = "/data/leichao/asd" + "/" + thisHospital;
			...
			for(File fileDelete : deleteChinese){
			    ...
			    String returnUrl= "http://222.24.62.190:8090/BB/jsps/test.jsp?";
				...
				}else returnUrl=" http://222.24.62.190:8090/BB/jsps/test.jsp?param0=000&param1=000&param2=000&param3=000&param4=000&param5=000";
				...
				returnUrl="";
				String asdHospital = "/data/leichao/asd" + "/" + thisHospital;		
				File uploadDir = new File(path);
		        if (!uploadDir.exists()) {
		        	System.out.println( "------path---uploadDir.exists()----");
		        	response.sendRedirect("http://222.24.62.190:8090/BB/jsps/test.jsp?param0=000&param1=000&param2=000&param3=000&param4=000&param5=000");	
		        	path=asdHospital;
		        	chosenIf=1;
		        }
					...
					if (!(path.contains("_"))) {
						...
						if (res != null) {
							...
							response.sendRedirect("http://222.24.62.190:8090/BB/jsps/test.jsp?param0="+chosenName[0]+"_"+thisDoctor+"&param1=000&param2=000&param3=000&param4=000&param5=000");
						}
						tmpChosenFile=null;
			...
			<%  
				file = new File(path);
			    try{
				copyFile(new File("/data/leichao/rename/"),file);
				...
				if(childs!=null&&childs.length!=0&&childs[0].getAbsolutePath().replaceAll("\\\\","/").contains(".")){
					String githubRaw = "http://222.24.62.190:8090/test";
					String[] resArray=new String[childs.length];
					String temp="/data/leichao/asd";
					for(int ii=0,jj=0;ii<childs.length;ii++,jj++){
						...
						if(jj == 0){						
							...
							if(tagDicomPath.exists()){
								...
								jsonPath=jsonPath.replace(temp,"http://222.24.62.190:8090/test");
								...
								commentsJsonPath=commentsJsonPath.replace(temp,"http://222.24.62.190:8090/test");
					...
					//response.sendRedirect("http://222.24.62.190:8090/BB/DWV/index.jsp");
					%>
					<script>
					
					parent.window.location.href='http://222.24.62.190:8090/BB/DWV/index.jsp?param0=<%=parameter[0] %>'
					</script>
					<%
				}
				
				updatePath=path;
				path = "/data/leichao/asd";
				...
				for (i = 0, m = 0; i < childs.length; i++, m++) {
				 if(childs[i].getName().contains("_mask")){					 
					...
					if(childs[i].getName().contains("_yes")){continue;}
                                        if(childs[i].getName().length()>40){continue;}
